...
let files = {};
 ...
 alert("Your account is created!");
   ...
   firebase.auth().onAuthStateChanged(function(user) {
      ...
      if (user) {
         ...
         storageRef.child("users/" + user.uid +'/data/').listAll().then(function(result){
            result.items.forEach(function(imageRef){
                ...
                showUsersStorageContectOnPage(i, imageRef,iiName,didsplayname);
         });
      });
      } else {
    ...
    function showUsersStorageContectOnPage(row, images,name,didsplayname) {
               images.getDownloadURL().then(function(URL) {
                  console.log(URL);
               let HTML = '';
               // HTML += '<a href="'+URL+'">'+name+'</a><p>'+row+'</p><img src="'+URL+'" alt="" width="100px" height="100px">';
               HTML+='<article class="storage-article"><div class="artcile-header"><div class="user-header-info"><img src="'+URL+'" alt="" class="user-profile-image"><b class="user-username">'+didsplayname+'</b></div><i class="fas fa-ellipsis-v"></i></div><img src="'+URL+'" alt="" class="image-of-user-storage"><div class="post-options"><div class="left-options"><a href='+URL+'><i class="fas fa-download"></i></a><i class="far fa-trash-alt" onclick="deletePostStorage()"></i></div><div class="right-options"><i class="far fa-bookmark"></i></div></div><div class="article-description"><b class="user-username">'+didsplayname+'</b><span class="post-name">'+name+'</span></div></article>';
